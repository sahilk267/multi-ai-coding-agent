...
import {
  ...
  ArrowRight, MessageSquare, Cpu, Zap, BarChart3,
} from "lucide-react";
...
const ROLE_META: Record<string, { label: string; icon: React.ComponentType<{ className?: string }>; color: string; model: string; desc: string }> = {
...
function StatusDot({ status }: { status: string }) {
  const color: Record<string, string> = {
    idle: "bg-zinc-500", running: "bg-green-400", waiting: "bg-amber-400",
    completed: "bg-emerald-400", failed: "bg-red-400", pending: "bg-zinc-600",
  };
  return (
    <span className={`inline-block h-2 w-2 rounded-full shrink-0 ${color[status] ?? "bg-zinc-500"} ${status === "running" ? "animate-pulse" : ""}`} />
  );
}

function AgentCard({ role, agent, isActive }: { role: string; agent?: AgentInfo; isActive: boolean }) {
  const meta = ROLE_META[role];
  const Icon = meta.icon;
  const status = agent?.status ?? "idle";

  return (
    <div className={`relative flex flex-col gap-2 p-4 rounded-xl border transition-all duration-300 ${
      isActive ? `${meta.color} shadow-lg` : "border-border/40 bg-card/30 text-muted-foreground"
    }`}>
      {isActive && (
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-mono font-semibold uppercase tracking-wider">{meta.label}</span>
        <StatusDot status={status} />
        <span className={`ml-auto text-[10px] font-mono ${STATUS_STYLE[status] ?? ""}`}>{status}</span>
      </div>
    ...
    <div className="border border-border/50 rounded-lg overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className={`text-[10px] font-mono flex items-center gap-1 ${meta?.color ?? ""} px-1.5 py-0.5 rounded border`}>
        ...
        <div className="px-4 pb-3 pt-1 bg-secondary/10 border-t border-border/30 space-y-2">
          ...
          {task.result && (
            <div className="rounded bg-background/60 border border-border/40 p-2">
              ...
              <p className="text-[11px] font-mono text-foreground whitespace-pre-wrap">{task.result.slice(0, 400)}</p>
            </div>
          )}
          {task.errorMessage && (
...
function MessageBubble({ msg }: { msg: AgentMessage }) {
  ...
  return (
    <div className={`flex gap-2 p-3 rounded-lg border text-xs ${typeStyle[msg.messageType] ?? "border-border/40 bg-card/20"}`}>
      <div className="flex items-center gap-1.5 shrink-0">
        <span className={`font-mono font-semibold ${fromMeta?.color?.split(" ")[0] ?? "text-muted-foreground"}`}>
          <FromIcon className="inline h-3 w-3 mr-1" />{msg.fromAgent}
        </span>
        <ArrowRight className="h-3 w-3 text-muted-foreground" />
        <span className={`font-mono font-semibold ${toMeta?.color?.split(" ")[0] ?? "text-muted-foreground"}`}>
          <ToIcon className="inline h-3 w-3 mr-1" />{msg.toAgent}
        </span>
      </div>
...
function PipelineFlowDiagram({ agents, activeRoles }: { agents: AgentInfo[]; activeRoles: Set<string> }) {
        ...
        return (
          <div key={role} className="flex items-center gap-1">
            <div className={`flex flex-col items-center gap-1 px-3 py-2 rounded-lg border transition-all ${
              ...
              <span className="text-[9px] font-mono uppercase tracking-wider">{meta.label}</span>
              {agent && <StatusDot status={agent.status} />}
            </div>
            {i < PIPELINE_STEPS.length - 1 && (
              <ArrowRight className={`h-3.5 w-3.5 shrink-0 ${isActive ? "text-primary" : "text-muted-foreground/20"}`} />
            )}
          </div>
        );
      })}
...
function useLiveFeed(sessionId: number | null, onUpdate: () => void) {
  ...
  const [liveMessages, setLiveMessages] = useState<Array<{ agent: string; level: string; message: string; timestamp: number }>>([]);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if (["agent_log", "agent_status", "agent_message", "pipeline_state", "pipeline_update"].includes(msg.type)) {
          const p = msg.payload ?? {};
          setLiveMessages(prev => [{
            agent: (p.agent as string) ?? msg.type,
            level: (p.level as string) ?? "info",
            message: (p.message as string) ?? JSON.stringify(p).slice(0, 120),
            timestamp: Date.now(),
          }, ...prev].slice(0, 200));
          if (["pipeline_update", "agent_status"].includes(msg.type)) onUpdate();
        }
      } catch { /* ignore */ }
    };
...
export function Pipeline() {
  ...
  const fetchStatus = async (sid: number) => {
    try {
  ...
  const liveMessages = useLiveFeed(sessionId, () => sessionId && fetchStatus(sessionId));
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      setStatus({ session: data.session, agents: data.agents, tasks: data.tasks, messages: [] });
    } catch (e: unknown) {
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <div className="flex flex-col w-80 shrink-0 border-r border-border overflow-hidden">
          ...
          <div className="shrink-0 p-4 border-b border-border space-y-3">
            <label className="text-xs font-mono text-muted-foreground uppercase tracking-wider">Goal</label>
            <Textarea
              value={goal}
            ...
            >
              ...
              {loading ? "Starting pipeline…" : "Launch Pipeline"}
            </Button>
            {error && (
          ...
          <div className="shrink-0 p-4 border-b border-border">
            <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider mb-3">Pipeline Flow</p>
            <PipelineFlowDiagram agents={status?.agents ?? []} activeRoles={activeRoles} />
          </div>

          {/* Agent cards */}
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider mb-2">Agents</p>
            {PIPELINE_STEPS.map(role => (
              <AgentCard
                key={role}
                role={role}
                agent={agentMap.get(role)}
                isActive={activeRoles.has(role) || agentMap.get(role)?.status === "completed"}
              />
            ))}
          </div>
        </div>
          ...
          {status && (
            <div className="shrink-0 px-5 py-3 border-b border-border flex items-center gap-3 bg-card/30">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-mono font-semibold truncate">{status.session.goal}</p>
                <p className="text-[10px] font-mono text-muted-foreground">Session #{status.session.id} · {status.session.status}</p>
              </div>
              <Badge variant="outline" className="text-[10px] font-mono shrink-0">
                {status.tasks.filter(t => t.status === "completed").length}/{status.tasks.length} tasks
              </Badge>
            </div>
          )}
            ...
            {!status && (
              <div className="flex flex-col items-center justify-center h-full text-muted-foreground gap-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs mt-1">Enter a goal and click "Launch Pipeline" to start the multi-agent system.</p>
                </div>
            ...
            {status && activeTab === "tasks" && (
              <div className="space-y-2">
                {status.tasks.length === 0 ? (
                  <p className="text-xs text-muted-foreground italic">No tasks yet — pipeline is initializing…</p>
                ) : (
            ...
            {status && activeTab === "messages" && (
              <div className="space-y-2">
                {status.messages.length === 0 ? (
                  <p className="text-xs text-muted-foreground italic">No inter-agent messages yet…</p>
                ) : (
                  status.messages.map(m => <MessageBubble key={m.id} msg={m} />)
                )}
              </div>
            )}
            ...
            {activeTab === "feed" && (
              <div className="space-y-1 font-mono text-[11px]">
                {liveMessages.length === 0 && (
                  <p className="text-muted-foreground italic">Waiting for live agent events…</p>
                )}
                {[...liveMessages].reverse().map((m, i) => (
                  <div key={i} className={`flex items-start gap-2 py-0.5 ${
                    m.level === "error" ? "text-red-400" : m.level === "warn" ? "text-amber-400" : "text-zinc-300"
                  }`}>
                    <span className="text-zinc-600 shrink-0 w-20">{new Date(m.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })}</span>
                    <span className={`shrink-0 w-12 ${ROLE_META[m.agent]?.color?.split(" ")[0] ?? "text-zinc-500"}`}>{m.agent.slice(0, 6)}</span>
                    <span className="flex-1 break-all">{m.message}</span>
                  </div>
                ))}
